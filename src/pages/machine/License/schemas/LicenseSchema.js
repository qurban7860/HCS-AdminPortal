...
import { pastDate, future20yearDate } from '../../util/index';
...
export const LicenseSchema = Yup.object().shape({
  ...
  .max(future20yearDate,`Extension Date field must be at earlier than ${fDate(future20yearDate)}`)
  .min(pastDate,`Extension Date field must be at after than ${fDate(pastDate)}`)
  .nullable().required('Extension Date is required'),

  requestTime: Yup.date()
  .typeError('Date Should be Valid')
  .max(future20yearDate,`Request Date field must be at earlier than ${fDate(future20yearDate)}`)
  .min(pastDate,`Request Date field must be at after than ${fDate(pastDate)}`)
