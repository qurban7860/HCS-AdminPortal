...
export default function ImagePreviewDialog({ onPreview, handleClosePreview, handleDownloadImage, imageName, imageExtension, file, imageData}) {
    return (
      <>
        <Dialog
        maxWidth="md"
        ...
        >
          <Typography variant="h4" title={`${imageName}.${imageExtension}`} sx={{ px: 1 , mr:5 , whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>
            {`${imageName}.${imageExtension}`}
          </Typography>{' '}
          <Link onClick={handleClosePreview} href="#" underline="none" 
            sx={{ 
                top: 15,
                right: 15,
                zIndex: 9,
