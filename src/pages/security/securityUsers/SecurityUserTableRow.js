    ...
    <TableRow hover selected={selected} >
      ...
      {lgScreen && !hiddenColumns?.['roles.name.[]'] && (
        <TableCell align="left" sx={{ textTransform: 'capitalize' }}>
          {roles.map((obj, index) =>
            obj.roleType === 'SuperAdmin' ? <Chip key={index} label={obj.name} sx={{ m: 0.2 }} color="secondary" /> : <Chip key={index} label={obj.name} sx={{ m: 0.2 }} />
          )}
        </TableCell>
      )}
      {!hiddenColumns?.['contact.firstName'] && (
        <TableCell align="left">
          {contact?.firstName && (
            <StyledTooltip
              placement="top"
              ...
              disableFocusListener
              tooltipcolor={contact.formerEmployee ? ICONS.FORMEREMPLOYEE.color : ICONS.NOTFORMEREMPLOYEE.color}
              color={contact.formerEmployee ? ICONS.FORMEREMPLOYEE.color : ICONS.NOTFORMEREMPLOYEE.color}
            >
              <Iconify icon={contact.formerEmployee ? ICONS.FORMEREMPLOYEE.icon : ICONS.NOTFORMEREMPLOYEE.icon} sx={{ mr: 1, height: 20, width: 20 }} />
            </StyledTooltip>
          )}
      ...
      {!hiddenColumns?.accountType && (
        <TableCell align="left">
          {isSPCustomer ? (
            <StyledTooltip title="SP User" placement="top" disableFocusListener tooltipcolor={theme.palette.primary.main} color="#1976d2">
              <Image
            ...
            <IconTooltip title="Customer User" color={ICONS.ACTIVE.color} icon="mdi:user-circle-outline" iconSx={{ border: 'none', ml: -0.7 }} />
          )}
        </TableCell>
      )}
