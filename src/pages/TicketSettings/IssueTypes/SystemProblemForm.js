...
import FormProvider, { RHFTextField, RHFUpload, RHFAutocomplete } from '../../../components/hook-form';
...
export default function SystemProblemForm() {
  ...
  const onSubmit = async (data) => {
    try {
      // await dispatch(addSystemProblem(data));
      reset();
      enqueueSnackbar('System Problem Send Successfully!');
  ...
  return (
    <Container maxWidth={false}>
      <StyledCardContainer>
        <Cover name="System Problem" ticketSettings />
      </StyledCardContainer>
            ...
            <Card sx={{ p: 3 }}>
              <Stack spacing={3}>
                ...
                >
                  ...
                  isOptionEqualToValue={(option, value) => option === value}
                  renderInput={(params) => ( <RenderCustomInput label="Share with*" params={params}/> )}
                  renderOption={(props, option) => ( <li {...props} key={option}> <span style={{ marginLeft: 8 }}>{option}</span> </li> )}
                  sx={{ maxWidth: 500 }}
                  />
                </Box>
