...
import { deleteDocument, getDocumentHistory , getDocument, getDocuments, setDocumentEditFormVisibility, setDocumentViewFormVisibility, setDocumentHistoryViewFormVisibility,resetDocument} from '../../../redux/slices/document/document';
...
export default function DocumentViewForm({ customerPage, machinePage, DocId }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      documentVersion:    (document?.documentVersions && document?.documentVersions?.length > 0 ) ? document?.documentVersions[0]?.versionNo : '',
  ...
  return (
    <Card sx={{ p: 3 }} >
      ...
      <Grid container>
          ...
          objectParam={
            defaultValues.documentVersion && (
              <Typography display="flex">
                ...
                {defaultValues.documentVersion &&
                  defaultValues.documentVersion > 1 && (
                    <VersionsLink
                      onClick={linkDocumentView}
        ...
        />
        { !customerPage && !machinePage && <><ViewFormField sm={6} heading="Customer" param={defaultValues?.customer} />
        ...
        <Grid item sx={{ display: 'flex-inline' }}>
          <Grid container justifyContent="flex-start" gap={1}>
            {document?.documentVersions && document?.documentVersions[0]?.files?.map((file) => (
              <Thumbnail
                key={file._id}
                file={file}
