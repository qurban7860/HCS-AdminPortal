...
// sections
import InvestigationReasonListTableRow from './InvestigationReasonListTableRow';
import InvestigationReasonListTableToolbar from './InvestigationReasonListTableToolbar';
import {
  getTicketInvestigationReasons,
  resetTicketInvestigationReasons,
  ChangeRowsPerPage,
  ChangePage,
  setFilterBy,
} from '../../../../../redux/slices/ticket/ticketSettings/ticketInvestigationReasons';
...
export default function InvestigationReasonList() {
  const { ticketInvestigationReasons, filterBy, page, rowsPerPage, isLoading, initial } = useSelector((state) => state.ticketInvestigationReasons);
  ...
  useLayoutEffect(() => {
    dispatch(getTicketInvestigationReasons(page, rowsPerPage));
    return () => {
  ...
  useEffect(() => {
    if (initial) {
      setTableData(ticketInvestigationReasons?.data || [] );
    }
  }, [ticketInvestigationReasons?.data, initial]);
    ...
    <Container maxWidth={false} >
      <StyledCardContainer>
       <Cover name="Investigation Reasons" supportTicketSettings/>
        ...
        <TableCard>
          <InvestigationReasonListTableToolbar
          ...
          {!isNotFound && <TablePaginationFilter
            count={ ticketInvestigationReasons?.totalCount || 0 }
            page={page}
              ...
              <Table size="small" sx={{ minWidth: 360 }}>
                ...
                <TableBody>
                  {(isLoading ? [...Array(rowsPerPage)] : dataFiltered)
                    .map((row, index) =>
                      row ? (
                        <InvestigationReasonListTableRow
                          key={row._id}
                          row={row}
