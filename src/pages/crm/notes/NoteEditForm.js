...
export default function NoteEditForm() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      reset();
      if(customerId && id ) await navigate(PATH_CRM.customers.notes.view(customerId, id))
    } catch (err) {
  ...
  const toggleCancel = () => { if(customerId && id ) navigate(PATH_CRM.customers.notes.view(customerId, id))};
