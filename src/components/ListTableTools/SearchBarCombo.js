...
function SearchBarCombo({
  ...
  return (
    <Grid container rowSpacing={logTypes?.length > 0 ? 2:1} columnSpacing={1} sx={{display:'flex', }}>
          { onChange && <Grid item xs={12} sm={12} md={12} lg={logTypes?.length > 0 || reduceFilterSize || (setAccountManagerFilter && setSupportManagerFilter) ? 4:6} xl={increaseFilterSize ? 10 : (logTypes?.length > 0 || reduceFilterSize || (setAccountManagerFilter && setSupportManagerFilter) ? 4:6)}> 
            <TextField
              fullWidth
              value={value}
              onChange={onChange}
