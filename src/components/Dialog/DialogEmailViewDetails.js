        ...
        <Grid>
          <Card sx={{ p: 3 }}>
            <Grid container>
              ...
              <ViewFormField sm={6} heading="Customer Name" param={email?.customer?.name || 'N/A'} isLoading={isLoading} />
              ...
              {email?.subject?.toLowerCase().includes('database backup') && (
                <ViewFormField sm={6} heading="DB Backup" isLoading={isLoading}
                  param={
                    <LinkTableCellWithIconTargetBlank
                      onViewRow={() => navigate(PATH_REPORTS.logs.dbBackup.root)}
                      onClick={() => window.open(PATH_REPORTS.logs.dbBackup.root, '_blank')}
                      param="Database Backup"
                      sx={{ ml: -2, mt: -2 }}
                    />
                  }
                />
